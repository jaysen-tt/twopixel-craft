...
> **CLI-first workflow (recommended):** Use `twopixel source ...` commands instead of editing source config files directly.
> - `twopixel source --help`
...
**Before doing anything else**, search for a specialized guide using the twopixels-docs MCP:

```
mcp__twopixels-docs__SearchCraftAgents({ query: "{service} source setup" })
```
...
Agent: [FIRST: Searches for Linear guide]
       mcp__twopixels-docs__SearchCraftAgents({ query: "linear source setup" })
...
`~/.twopixel/provider-domains.json`
...
1. Create the source folder:
   ```bash
   mkdir -p ~/.twopixel/workspaces/{ws}/sources/my-source
   ```
