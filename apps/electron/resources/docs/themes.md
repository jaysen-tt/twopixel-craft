...
3. **Preset themes**: `~/.twopixel/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.twopixel/theme.json` - Override specific colors (app-level)
...
```
~/.twopixel/workspaces/{id}/config.json
```
...
Create `~/.twopixel/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `~/.twopixel/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `~/.twopixel/themes/{name}.json`
...
1. Create `~/.twopixel/theme.json` for overrides or `~/.twopixel/themes/{name}.json` for a preset
...
- Check file is in correct location (`~/.twopixel/theme.json` for overrides, `~/.twopixel/themes/` for presets)
