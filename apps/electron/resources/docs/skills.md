...
> **CLI-first workflow (recommended):** Use `twopixel skill ...` commands instead of editing `SKILL.md` files directly.
> - `twopixel skill --help`
...
1. **Workspace skill checked first** - If `~/.twopixel/workspaces/{id}/skills/commit/SKILL.md` exists, it's used
...
```
~/.twopixel/workspaces/{workspaceId}/skills/{slug}/
...
```bash
mkdir -p ~/.twopixel/workspaces/{ws}/skills/my-skill
```
...
1. Create `~/.twopixel/workspaces/{ws}/skills/commit/SKILL.md`
